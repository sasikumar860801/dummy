...
<head>
    ...
    <title>{{ $stock->meta_title ?? $stock->model_title . ' (' . $stock->capacity . ') - Buy Refurbished' }}</title>
    <meta name="description" content="{{ $stock->meta_description ?? 'Buy a high-quality refurbished ' . $stock->model_title . ' at an unbeatable price.' }}">
    <meta name="keywords" content="{{ $stock->meta_keywords ?? 'refurbished phone, second hand ' . $stock->model_title }}">
    <meta name="robots" content="{{ $stock->meta_robots ?? 'index, follow' }}">
    @if(!empty($stock->canonical_url))
        <link rel="canonical" href="{{ $stock->canonical_url }}">
    @endif

    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">
    <style>
        body { background-color: #050508; color: #cbd5e1; font-family: system-ui, -apple-system, sans-serif; margin: 0; padding: 40px 20px; }
        .product-container { max-width: 1200px; margin: 0 auto; display: grid; grid-template-columns: 1.2fr 1fr; gap: 50px; background: #111118; border: 1px solid #1e1e2a; border-radius: 24px; padding: 40px; }
        
        /* Left Side: E-commerce Vertical Media Split */
        ...
        .thumb-nav-card { width: 75px; height: 75px; object-fit: cover; border-radius: 10px; border: 2px solid #1e1e2a; background: #0c0c14; cursor: pointer; transition: 0.2s; flex-shrink: 0; }
        .thumb-nav-card:hover, .thumb-nav-card.active { border-color: #ec4899; }
        
        .main-display-viewport { flex: 1; background: #0c0c14; border: 1px solid #1e1e2a; border-radius: 16px; display: flex; align-items: center; justify-content: center; position: relative; overflow: hidden; }
        .main-display-viewport img, .main-display-viewport video { max-width: 100%; max-height: 100%; object-fit: contain; }

        /* Right Side: Product Meta & Specifications */
        ...
        .buy-now-action-btn { background: linear-gradient(135deg, #ec4899, #8b5cf6); color: white; border: none; padding: 16px 32px; font-size: 18px; font-weight: 700; border-radius: 12px; cursor: pointer; transition: 0.2s; display: inline-flex; align-items: center; justify-content: center; gap: 10px; box-shadow: 0 10px 20px -5px rgba(236, 72, 153, 0.4); }
        .buy-now-action-btn:hover { opacity: 0.95; transform: translateY(-2px); }

        /* Recommendations Strip Footer Blueprint */
        .recommendations-section { max-width: 1200px; margin: 60px auto 0 auto; text-align: left; }
        ...
        .recommendations-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 24px; }
        ...
        .rec-card:hover { border-color: #2a2a3a; transform: translateY(-3px); }
    ...
    <div class="media-gallery-showcase">
        <div class="thumbnail-track-column">
            @foreach($mediaGallery as $index => $mediaPath)
                @if(preg_match('/\.(mp4|webm|avi|mov)$/i', $mediaPath))
                    <video src="/{{ $mediaPath }}" class="thumb-nav-card {{ $index === 0 ? 'active' : '' }}" onclick="updateStageMedia(this, true)" muted></video>
                @else
                    <img src="/{{ $mediaPath }}" class="thumb-nav-card {{ $index === 0 ? 'active' : '' }}" onclick="updateStageMedia(this, false)">
                @endif
            @endforeach
        </div>
        ...
        <div class="main-display-viewport" id="stage_viewport_frame">
            @if(preg_match('/\.(mp4|webm|avi|mov)$/i', $mediaGallery[0] ?? ''))
                <video src="/{{ $mediaGallery[0] ?? '' }}" id="stage_media_element" controls autoplay muted></video>
            @else
                <img src="/{{ $mediaGallery[0] ?? '' }}" id="stage_media_element">
            @endif
        </div>
    </div>
...
<div class="recommendations-section">
    ...
    <div class="recommendations-grid">
        @forelse($relatedStocks as $rel)
            @php 
                // FIX: Fallback directly to model_img instead of testing media gallery
            ...
            <a href="/buy-refurbished-mobile-phones/buy-{{ $rel->sef_url }}" class="rec-card">
                <div>
                    <div class="rec-img-holder">
                        <img src="/{{ $thumbSrc }}" alt="{{ $rel->model_title }}">
                    </div>
