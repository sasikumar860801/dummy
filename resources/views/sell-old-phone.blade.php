...
<div class="container">
    ...
    <div style="padding: 20px 0; font-size: 14px;">
        <nav aria-label="breadcrumb">
            <ol style="display: flex; gap: 8px; list-style: none; flex-wrap: wrap;">
                ...
                <li><a href="{{ url('/sell-old-phone') }}" style="color: #3b82f6; text-decoration: none;">Sell Old Phone</a></li>
                <li><i class="fas fa-chevron-right" style="font-size: 10px; color: #64748b;"></i></li>
    ...
    <div style="text-align: center; margin-bottom: 40px;">
        <h1 style="font-size: 36px; font-weight: 800; margin-bottom: 16px; background: linear-gradient(135deg, #60a5fa, #a78bfa); -webkit-background-clip: text; background-clip: text; color: transparent;">
            Sell {{ $brand->title }} Phone
        </h1>
        <p style="color: #94a3b8; max-width: 600px; margin: 0 auto;">
            Get instant best price for your used {{ $brand->title }} mobile. Free doorstep pickup, instant cash payment.
        </p>
    </div>
    ...
    <div>
        <h3 style="font-size: 20px; font-weight: 700; margin-bottom: 20px;">Choose Model</h3>
        <div id="modelsGrid" style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 20px;">
            @foreach($models as $model)
            <div class="model-card" data-series-id="{{ $model->model_series_id }}" data-model-title="{{ strtolower($model->title) }}">
                <div style="background: #111118; border: 1px solid #2a2a3a; border-radius: 16px; padding: 20px; text-align: center; transition: all 0.3s; cursor: pointer;">
                    <!-- Model Image -->
                    <div style="width: 100px; height: 100px; margin: 0 auto 15px; display: flex; align-items: center; justify-content: center;">
                        @if($model->model_img && file_exists(public_path('media/images/model/' . $model->model_img)))
                            <img src="{{ $model->model_img_url }}" alt="{{ $model->title }}" style="max-width: 90px; max-height: 90px; object-fit: contain;">
                        @else
                            <i class="fas fa-mobile-alt" style="font-size: 50px; color: #3b82f6;"></i>
                        @endif
                    </div>
                    ...
                    <!-- Price -->
                    @if($model->sell_price)
                    <p style="font-size: 18px; font-weight: 700; color: #3b82f6;">₹{{ number_format($model->sell_price) }}</p>
                    @endif
        ...
        <!-- No results message -->
        <div id="noResults" style="display: none; text-align: center; padding: 60px; background: #111118; border-radius: 20px;">
            <i class="fas fa-search" style="font-size: 50px; color: #64748b; margin-bottom: 20px;"></i>
            <h3 style="color: white;">No models found</h3>
